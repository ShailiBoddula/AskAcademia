...
function App() {
  ...
  const cards = [
    ...
    },
    {
      ...
      title: "Campus",
      subtitle: "Recruitment Notices",
      description: "and Placement Updates",
        ...
        >
          ...
          >
            {chatHistory.map((chat, index) => (
              ...
              >
                {/* Structured Notice Cards */}
                {chat.notices && chat.notices.length > 0 ? (
                  <div className="space-y-3">
                    {chat.notices.map((notice, i) => (
                      <div key={i} className="border border-gray-200 rounded-lg p-3 bg-white">
                        <div className="font-semibold text-gray-900 mb-1">{notice.title}</div>
                        <div className="text-xs text-gray-500 mb-2">{notice.date}</div>
                        <div className="text-sm text-gray-700 mb-2 line-clamp-3">{notice.body}</div>
                        {notice.pdf_link && (
                          <a 
                  ...
                  <p className="text-[13px] md:text-base whitespace-pre-wrap break-words">
                    {chat.text}
                  </p>
                )}
              </div>
